...
function guardarUsuarioMovil(data) {
  for (var item in data) {
    if (item == 'data') {
      if (data[item].length != 0) {
        for (x = 0; x < data[item].length; x++) {
          fetch('http://192.168.1.22:8080/web_service/insert_usuario_movil.php?group_name=' + data[item][x].group_name + '&profile_name=' + data[item][x].profile_name + '&user_name=' + data[item][x].user_name + '&last_name=' + data[item][x].last_name + '&login=' + data[item][x].login + '&movil_assign_user=' + data[item][x].movil_assign_user)
            .then(data => {
              return data.json()
            })
            .then(data => {
...
function guardarGrupoMovil(data) {
  for (var item in data) {
    if (item == 'data') {
      if (data[item].length != 0) {
        for (x = 0; x < data[item].length; x++) {
          fetch('http://192.168.1.22:8080/web_service/insert_grupo_movil.php?Id=' + data[item][x].id + '&name=' + data[item][x].name + '&group_name=' + data[item][x].group_name + '&description=' + data[item][x].description)
            .then(data => {
              return data.json()
            })
            .then(data => {
