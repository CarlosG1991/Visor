...
$a             = "'";
$datos         = array();
$consultaMovil = 'select "imei" from administration.device;';
$sqlMovil      = pg_query($conexion, $consultaMovil);
if ($sqlMovil) {
    while ($objMovil = pg_fetch_object($sqlMovil)) {
        $consulta = "select * from console.packet_data_tmp WHERE gps_date_time BETWEEN " . $a . $desde . $a . " AND " . $a . $hasta . $a . ' order by gps_date_time asc;';
            ...
            if ($rows != 0) {
                while ($obj = pg_fetch_object($sql)) {
                    ...
                    while ($objA = pg_fetch_object($sqlAle)) {
                        $fecha   = explode(" ", $obj->gps_date_time);
                        $datos[] = array(
                            'unit'  => $obj->unit_id,
                            'fecha' => $fecha[1],
                            'alert' => $objA->Nombre,
